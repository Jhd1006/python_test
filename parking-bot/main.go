...
var (
	...
	lotMutex    sync.Mutex
	maxSlots    = 1000 // DB에 생성한 슬롯 100개
	//fixedZoneID = 1   // DB에 생성한 Zone ID 1
)

// 🚀 [수정] DB에 미리 등록한 100가1001 ~ 2000 번호만 생성
func generateRegisteredPlate() string {
	// 1부터 1000 사이의 숫자 생성 (001 ~ 1000)
...
func main() {
	...
	for {
		...
		lotMutex.Unlock()
		time.Sleep(3 * time.Second) // 너무 빠르지 않게 조절
	}
}
